    ...
    public void Configure(EntityTypeBuilder<AnnouncementAudience> builder)
    {
        builder.ToTable("announcement_audiences", table =>
        {
            table.HasCheckConstraint(
                ...
                "(audience_type IN ('ALL_USERS', 'ALL_RESIDENTS') AND role_id IS NULL AND apartment_unit_id IS NULL AND resident_id IS NULL) OR " +
                "(audience_type = 'ROLE' AND role_id IS NOT NULL AND apartment_unit_id IS NULL AND resident_id IS NULL) OR " +
                "(audience_type = 'APARTMENT' AND role_id IS NULL AND apartment_unit_id IS NOT NULL AND resident_id IS NULL) OR " +
                "(audience_type = 'RESIDENT' AND role_id IS NULL AND apartment_unit_id IS NULL AND resident_id IS NOT NULL)");
        });
