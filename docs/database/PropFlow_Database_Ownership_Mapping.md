...
| **3** | `auth` | `password_reset_tokens` | `Authentication` | `PasswordResetToken` | Child (`UserAccount`) | `id` (uuid) | `UNIQUE(token_hash)`; salted OTP hash, `attempt_count`; successful OTP creates hashed one-use `proof_hash` with `proof_expires_at` and `verified_at` | None | Pending OTP → verified reset proof → Expired / Used (`used_at`) |
